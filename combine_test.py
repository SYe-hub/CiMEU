...
from models.ETH_Net_Multi import eth_net
import matplotlib
...
fm.fontManager.addfont('/data3/LM/Times/times.ttf')
...
parser.add_argument('--resume_norm',
                    default='/data3/LM/result/PTH-Net-Pro/checkpoint/Best/DFEW-norm-1-model_best.pth',
                    type=str, metavar='PATH', help='path to normal checkpoint')
parser.add_argument('--resume_rv',
                    default='/data3/LM/result/PTH-Net-Pro/checkpoint/Best/DFEW-rv-1-model_best.pth',
...
parser.add_argument('--freqm', help='frequency mask max length', type=int, default=24)
...
parser.add_argument("--audio_length", type=int, default=256, help="the dataset spectrogram std")
...
color_list = [
    'gold', 'silver', 'rose gold', 'mint green',
    'navy blue', 'olive green', 'dark purple',
    'dark blue', 'dark red', 'peach'
]

# rgb_values = [
#     (255, 215, 0), (192, 192, 192), (255, 192, 203),
#     (152, 255, 152), (0, 0, 128), (128, 128, 0),
#     (128, 0, 128), (0, 0, 102), (139, 0, 0),
#     (255, 228, 196)
# ]
rgb_values = [
    (220, 20, 60),  # crimson (对比鲜明的红色)
    (65, 105, 225),  # royal blue (深蓝色)
    (34, 139, 34),  # forest green (森林绿)
    (255, 165, 0),  # orange (亮橙色)
    (75, 0, 130),  # indigo (靛蓝色)
    (148, 0, 211),  # dark violet (深紫色)
    (255, 105, 180),  # hot pink (亮粉色)
    (139, 69, 19),  # saddle brown (深棕色)
    (169, 169, 169),  # dark gray (深灰色)
    (0, 255, 255),  # bright cyan (亮青色)
    (0, 0, 205)  # medium blue (中蓝色)
]

def plot_embedding(data, label, n_clusters):
    ...
    for i in range(data.shape[0]):
        ...
        normalized_rgb = [tuple(value / 255 for value in rgb) for rgb in rgb_values]
        plt.plot(data[i, 0], data[i, 1], 'o', markersize=2, color=normalized_rgb[label[i]])
    ...
    #     plt.scatter(centroid[0], centroid[1], s=100, color='red', marker='v')
...
def plot_embedding_with_boundary(data, label, n_clusters):
    x_min, x_max = np.min(data, 0), np.max(data, 0)
    data = (data - x_min) / (x_max - x_min)
    ...
    # 绘制类别边界
    plt.scatter(data[:, 0], data[:, 1], c=label, s=20, cmap='viridis', alpha=0.5)
    ...
    save_path = f'fig/{args.dataset}/{args.data_set}.png'
    if not os.path.exists(os.path.dirname(save_path)):
        os.makedirs(os.path.dirname(save_path))
...
def main():
    ...
    model_norm = eth_net(args, args.n_in, args.n_embd, args.mlp_dim, args.max_len, args.arch, args.scale_factor, args.with_ln,
    ...
    model_rv = eth_net(args, args.n_in, args.n_embd, args.mlp_dim, args.max_len, args.arch, args.scale_factor, args.with_ln,
                       args.path_pdrop, args.downsample_type, args.thr_size, args.k, use_pos=args.use_pos,
                       num_classes=args.num_class)
    model_rv = torch.nn.DataParallel(model_rv).cuda()
    model_flow = eth_net(args, args.n_in, args.n_embd, args.mlp_dim, args.max_len, args.arch, args.scale_factor, args.with_ln,
                         args.path_pdrop, args.downsample_type, args.thr_size, args.k, use_pos=args.use_pos,
                         num_classes=args.num_class)
    ...
    # Data loading code
    val_data_norm = test_data_loader(args, dataset=args.dataset, data_mode='norm', data_set=args.data_set,
    ...
    val_loader_norm = torch.utils.data.DataLoader(val_data_norm,
                                                  ...
                                                  pin_memory=True)
    val_data_rv = test_data_loader(args, dataset=args.dataset, data_mode='rv', data_set=args.data_set, is_face=args.is_face,
                                   label_type=args.label_type)
    ...
    if args.resume_rv and args.resume_flow:
        ...
        #                             num_label=args.num_class)
        result, tar, c_m = validate_1(val_loader_norm, val_loader_rv, None, model_norm, model_rv, None, mode=1,
                                    num_label=args.num_class)
    else:
    ...
    if args.label_type == 'single':
        ...
        plot_embedding(result, tar, args.num_class)
        # plot_embedding_with_boundary(result, tar, args.num_class)
    plt.clf()
    # Confusion matrix
    # CM = []
    # for row in c_m:
    ...
    #     CM.append(row * 100)
    # ax = seaborn.heatmap(
    #     CM, xticklabels=emotions, yticklabels=emotions, cmap='YlGn', annot=True, fmt='.2f')
...
def validate(val_loader_norm=None, val_loader_rv=None, val_loader_flow=None, model_norm=None, model_rv=None,
    ...
    with torch.no_grad():
        if mode == 2:
            ...
            for i, (
            (input_H_norm, input_M_norm, input_L_norm, target_norm, audio_norm), (input_H_rv, input_M_rv, input_L_rv, target_rv, audio_rv)) \
                ...
                pred_norm = model_norm(None, None, input_H_norm, None, None, masks_H_norm, False, audio_norm)
                ...
                pred_rv = model_rv(None, None, input_H_rv, None, None, masks_H_rv, False, audio_rv)
            ...
            model_norm.eval()
            file_data = ""
            for i, (input_H, input_M, input_L, target, audio) in enumerate(val_loader_norm):
                ...
                pred = model_norm(None, None, input_H, None, None, masks_H, False, audio)
                Result.extend(pred.detach().cpu().numpy())
                # measure accuracy and record loss
